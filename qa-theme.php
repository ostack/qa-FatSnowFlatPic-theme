...
class qa_html_theme extends qa_html_theme_base
{
	...
	public function q_list_item($q_item)
	{
		...
		$this->q_query_content($q_item);
        $this->q_item_picture($q_item);
	...
	public function q_item_picture($q_item)
	{   $content = $q_item['raw']['content'];
		if (!empty($content)) {
			preg_match_all("/<img[^<>]*src=[\"]([^\"]+)[\"][^<>]*>/im",$content,$matches);  
	        $img_url = $matches[1];
			if(!empty($img_url)){
				$this->output('<div class="qa-q-item-picture">');
				$this->output('<img src="'.$img_url[0].'">');
				$this->output('</div>');
			}else{
	...
	{
		//echo print_r($q_item['raw']);
		
		$content = isset($q_item['content']) ? $q_item['content'] : '';
		$this->output('<div class="qa-q-view-content">');
		$this->output_raw($this->q_list_content($q_item));
		$this->output('</div>');
	...
	public function attribution()
	{
		...
		$this->output(
			...
			'<a href="https://paypal.me/guangyuezhao">Donate</a>',
			'</div>'
		);
		$this->output(
			'<div class="qa-attribution">',
			'Theme by <a href="http://www.q2amarket.com">Q2A Market</a>&&<a href="http://www.OStack.cn">OStack.cn</a>',
			'</div>'
		);
	...
	private function qam_user_account()
	{
		if (qa_is_logged_in()) {
			...
			$handle = '';
			if(isset($this->content['navigation']['user']['login'])){
	...
	public function q_list_content($q_item)
	{
		$blockwordspreg = qa_get_block_words_preg();
		if (!empty($q_item['raw']['content'])) {
			
			//echo print_r($q_item['raw']['content']);
			$text = qa_viewer_text($q_item['raw']['content'], $q_item['raw']['format'], array('blockwordspreg' => $blockwordspreg));
			$text = preg_replace('/\s+/', ' ', $text);  // Remove duplicated blanks, new line characters, tabs, etc
			$text = qa_shorten_string_line($text, 240);
			return "&nbsp;&nbsp;&nbsp;&nbsp;".qa_html($text).'...';
		}else{
			//echo print_r($q_item['raw']['content']);
		}
	...
	public function q_query_content(&$q_item)
	{
		if (!empty($q_item['raw']['postid'])) {
			$result = qa_db_query_sub('SELECT postid, content, format FROM ^posts WHERE postid = #', $q_item['raw']['postid']);
			$postinfo = qa_db_read_all_assoc($result, 'postid');
			//echo print_r($postinfo[$q_item['raw']['postid']]);
			$q_item['raw']['format']=$postinfo[$q_item['raw']['postid']]['format'];
			$q_item['raw']['content']=$postinfo[$q_item['raw']['postid']]['content'];
		}
	}

	public function post_avatar_meta($post, $class, $avatarprefix = null, $metaprefix = null, $metaseparator = '&nbsp;&nbsp;')
	{
